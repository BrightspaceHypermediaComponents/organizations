...
class Entity {
	 ...
	 */
	dispose() {
		this._subEntities.forEach(entity => factory.dispose(entity));
	...
	_subEntity(entityType, href, onChange) {
		...
		if (this._subEntities.has(href)) {
			factory.dispose(this._subEntities.get(href));
		}
		factory.entityFactory(entityType, href, this._token, (entity) => {
